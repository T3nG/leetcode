var threeSum = function(nums) {
  ...
  negatives.forEach((neg, i) => {
    ...
    while (j+1 < negatives.length){
      ...
      let arr = [neg, negatives[j+1], complement].sort()
      if (uPos.includes(complement) && !result.some(
  ...
  positives.forEach((pos, i) => {
    ...
    while (j+1 < positives.length){
      ...
      let arr = [pos, positives[j+1], complement].sort()
      if (uNeg.includes(complement) && !result.some(
        element => JSON.stringify(element) === JSON.stringify(arr)
        )) {
        result.push(arr)
      }
